...
linkml_meta = LinkMLMeta(
    {
        ...
        "source_file": "/home/harry/ontogpt/src/ontogpt/templates/aop.yaml",
...
class AOPPathway(ConfiguredBaseModel):
    ...
    molecular_initiating_event: Optional[MolecularInitiatingEvent] = Field(
        default=None,
        description="""A free-text description of the molecular initiating event (MIE) that starts the adverse outcome pathway. Include all details necessary to understand the MIE, such as the stressor involved, the biological target, and the biological level at which the MIE occurs.""",
        json_schema_extra={
            "linkml_meta": {"alias": "molecular_initiating_event", "domain_of": ["AOPPathway"]}
        },
    )
    key_events: Optional[list[KeyEvent]] = Field(
        default=None,
        description="""A semicolon-delimited list of the sequence of key events (KEs) that link the molecular initiating event to the adverse outcome. For each key event, include a free-text description that captures the biological target, event type, and biological level at which the key event occurs.""",
        json_schema_extra={"linkml_meta": {"alias": "key_events", "domain_of": ["AOPPathway"]}},
    )
    adverse_outcome: Optional[list[AdverseOutcome]] = Field(
        default=None,
        description="""A semicolon-delimited list of the final adverse outcome (AO) or outcomes. This may be a disease, phenotype, or impact on an organism or population.""",
        json_schema_extra={
            "linkml_meta": {"alias": "adverse_outcome", "domain_of": ["AOPPathway"]}
        },
    )
...
class MolecularInitiatingEvent(ConfiguredBaseModel):
    ...
    target: Optional[str] = Field(
        default=None,
        description="""The biological molecule or structure that participates in the event. This may be a gene, protein, or other biomolecule (e.g., PARP1) or a chemical species (e.g., reactive oxygen species).""",
        json_schema_extra={
            "linkml_meta": {
                "alias": "target",
                "domain_of": ["MolecularInitiatingEvent", "KeyEvent"],
            }
        },
    )
    event_type: Optional[EventTypeEnum] = Field(
        default=None,
        description="""The type of event. Must be one of the following: activation, inhibition, increase, decrease, impairment, agonism, antagonism, formation, alkylation, phosphorylation, failure, or occurrence. For diseases or phenotypes, the event type is often \"increase\", representing an increase in the severity or prevalence of the condition. Occurrence is a general event type that should be used when other types do not apply.""",
        json_schema_extra={
            "linkml_meta": {
                "alias": "event_type",
    ...
    biological_level: Optional[LevelOfBiologicalOrganizationEnum] = Field(
        default=None,
        description="""The level of biological organization at which the event occurs. Must be one of the following: molecular, cellular, tissue, organ, organism, individual, or population.""",
        json_schema_extra={
            "linkml_meta": {
    ...
    short_name: Optional[str] = Field(
        default=None,
        description="""A short name or identifier for the event. This is generally a combination of the target and event type, such as \"Formation, Pro-mutagenic DNA Adducts\" or \"Increase, reactive oxygen species\".""",
        json_schema_extra={
            "linkml_meta": {
                "alias": "short_name",
                "domain_of": ["MolecularInitiatingEvent", "KeyEvent", "AdverseOutcome"],
    ...
    stressor: Optional[str] = Field(
        default=None,
        description="""The chemical or physical agent that initiates the molecular initiating event. In the case of a chemical stressor, this may be referred to as a Chemical Initiator (CI). If the stressor is not known, it may be referred to as an Unknown Initiator (UI), so the value for this attribute should be \"UI\". Examples: aflatoxin B1, lead, radiation.""",
        json_schema_extra={
            "linkml_meta": {"alias": "stressor", "domain_of": ["MolecularInitiatingEvent"]}
        },
    )
...
class AdverseOutcome(ConfiguredBaseModel):
    ...
    outcome: Optional[str] = Field(
        default=None,
        description="""The adverse outcome or effect that results from the key events in the pathway.""",
        json_schema_extra={"linkml_meta": {"alias": "outcome", "domain_of": ["AdverseOutcome"]}},
    )
    ...
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta(
        {
            "annotations": {
                "annotators": {
                    "tag": "annotators",
                    "value": "sqlite:obo:chebi, sqlite:obo:envo, " "sqlite:obo:opmi",
                },
                "prompt": {
                    "tag": "prompt",
...
class BiologicalTarget(NamedEntity):
    ...
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta(
        {
            "annotations": {
                "annotators": {"tag": "annotators", "value": "sqlite:obo:go, sqlite:obo:ncit"}
            },
            "from_schema": "http://w3id.org/ontogpt/adverse_outcome_pathway",
            "id_prefixes": ["GO", "NCIT"],
        }
...
class Outcome(NamedEntity):
    ...
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta(
        {
            "annotations": {
                "annotators": {"tag": "annotators", "value": "sqlite:obo:mondo, sqlite:obo:ncit"}
            },
            "from_schema": "http://w3id.org/ontogpt/adverse_outcome_pathway",
...
class CellTerm(NamedEntity):
    ...
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta(
        {
            "annotations": {"annotators": {"tag": "annotators", "value": "sqlite:obo:cl"}},
            "from_schema": "http://w3id.org/ontogpt/adverse_outcome_pathway",
            "id_prefixes": ["CL"],
        }
    )
